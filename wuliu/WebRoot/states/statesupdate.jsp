...
<script language="javascript" type="text/javascript" src="js/jquery.min.js"></script>
	...
	<body>
<form action="method!statesupdate2" method="post"  onsubmit="return checkform()">
    ...
    <div id="regdiv">
        ...
        <div class="box">
         ...
         <c:forEach items="${list}" var="bean2">
          <option value="${bean2.id }"<c:if test="${bean2.id==bean.audit.states.user.id }">selected</c:if> >${bean2.truename}</option>
         </c:forEach>
                  ...
                  <tr >
                  <td class="td_right">指定派送车辆:</td>
                    <td class="">
                    <span class="fl">
                      <div class="select_border "> 
                        <div class="select_containers "> 
                         <select name="car" class="select" id="carid">
                       <option value="">--请选择--</option>
         <c:forEach items="${list2}" var="bean2">
          <option value="${bean2.id }"<c:if test="${bean2.id==bean.audit.states.car.id }">selected</c:if> >${bean2.bianhao}</option>
         </c:forEach>
         ...
         <option value="" >--请选择--</option>
         <option value="开始派送" <c:if test="${bean.audit.states.send=='开始派送' }">selected</c:if> >开始派送</option>
